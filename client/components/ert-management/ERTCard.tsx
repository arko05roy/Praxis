...
export function ERTCard({ ertId, onClick }: ERTCardProps) {
    ...
    const { data: estimatedPnl, isProfitable } = useEstimatePnl(ertId);

    if (isLoading || !rights) {
        return (
            <div className="glass-panel p-6 rounded-2xl h-[280px] animate-pulse flex items-center justify-center">
    ...
    const canSettle = canSettleResult?.canSettle || isExpired;
    const isSettled = rights.ertStatusName === 'SETTLED';
                ...
                <div className="space-y-4">
                    ...
                    {!isSettled && (
                        <button
                            ...
                            disabled={!canSettle && !isExpired}
                        >
                            <Banknote className="w-4 h-4" />
                            {isExpired ? "Settle Expired ERT" : canSettle ? "Settle Now" : "Not Ready to Settle"}
                        </button>
                    )}

                    {isSettled && (
                        <div className="w-full py-2.5 rounded-xl font-medium text-sm flex items-center justify-center gap-2 bg-blue-500/10 text-blue-500 border border-blue-500/20">
